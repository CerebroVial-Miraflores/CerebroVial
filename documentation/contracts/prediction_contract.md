...
- **Persistencia de modelos**: los **4 artefactos** entrenados (uno por dirección) se serializan como **state_dicts** (`torch.save` de un dict con la clave `state_dict` + metadatos, **no** el modelo pickleado completo) a una **ubicación fija documentada** bajo `ia_prediction_service/` (el path exacto lo fija CT-09.3 al implementar TTH-09). Los binarios van por **Git LFS** (convención del repo para `.pt`/`.joblib`/`.ckpt`).
- **Mecanismo de carga y serving (D-010)**: el núcleo carga los **4 `.pt` al arranque** (patrón **eager**, análogo a cómo el baseline RandomForest carga sus `.joblib` al inicializarse — CT-09.4.a), no por request. La carga usa **`torch` CPU-only** (instalado en el core vía `--index-url https://download.pytorch.org/whl/cpu`; ver D-010) y la clase **`GRUMultiOutput` vendorizada en el core**: como el artefacto es un state_dict, cargar requiere **instanciar la clase y luego `load_state_dict`** (con `map_location="cpu"`), por lo que la definición `nn.Module` debe estar disponible en el proceso del núcleo. La inferencia es **síncrona e in-process** (4 forwards, uno por dirección); no hay servicio de predicción por red. Esto **revisa SAN-01** de forma acotada: torch entra al core solo para esta inferencia, no para entrenar ni en `cerebrovial_shared` — ver `documentation/lean-inception/4-decisiones/DECISIONS.md` § D-010.
- **Paridad clase ↔ artefacto**: la `GRUMultiOutput` vendorizada en el core debe ser **byte-compatible** con el state_dict que produce `ia_prediction_service` — es decir, **el mismo `nn.Module`** (mismas capas, nombres de parámetros y shapes: `nn.GRU(input_size=1, hidden_size, num_layers=1)` + `nn.Linear(hidden, horizonte × n_classes)`). Una divergencia entre la clase vendorizada y la de entrenamiento haría fallar `load_state_dict` (o, peor, cargaría pesos a una arquitectura distinta en silencio). Esta compatibilidad se verifica con un **test de paridad** (se implementará en **Fase 3a**) que carga los `.pt` reales contra la clase vendorizada y comprueba que el `load_state_dict` es exacto (sin claves faltantes/sobrantes) y que un forward de referencia reproduce la salida esperada.
