...
| DHU-028 | Ampliación del Product Backlog: vista panorámica de red por tramo de vía (HU-22 + TTH-12). El backlog funcional pasa de 21 HU + 11 TTH a 22 HU + 12 TTH; HU-22 usa la unidad "tramo de vía/arista"; HU-03/HU-04 se reinterpretan como vista de detalle complementaria (sin reabrirse) | 2026-06-02 | Cerrada |
| DHU-029 | Ampliación del Product Backlog: recorrido temporal de la congestión de la red (HU-23 + TTH-13). El backlog funcional pasa de 22 HU + 12 TTH a 23 HU + 13 TTH; HU-23 agrega el eje temporal de recorrido sobre el mapa de HU-22 (misma unidad tramo de vía/arista), habilitada por TTH-13 (consulta histórica de congestión por arista: estado por instante + rango disponible); reutiliza la infraestructura de TTH-12 sin reabrir HU-22 ni HU-03/HU-04 | 2026-06-02 | Cerrada |
...
## DHU-026 — Persistencia en el worker del `AsyncAggregator`: §11 supersede de §4.4 Cambio 2 en async-only MVP1 (CERRADA)

**Fecha:** 2026-05-28.
**Estado:** Cerrada.
**TTH afectada:** TTH-08, Fase 5 (paso 0 de 5b).
**Decisiones que extiende:** DHU-024 (encuadre TTH-08), DHU-025 (extensión de
`ZoneVehicleCount`). Reabre la **Fase 3** únicamente para corregir el docstring
del Protocol `AsyncAggregator` y la sección §4.4 Cambio 2 del documento de
diseño.

### Contexto

Al iniciar el paso 0 de 5b (reconciliación §4.4 Cambio 2 vs §11), la auditoría
detectó que el documento `tth-08-fase1-diseno.md` y el Protocol del dominio
`AsyncAggregator` (`edge_device/src/vision/domain/protocols.py`) contienen un
**conflicto interno no resuelto explícitamente** sobre quién persiste en modo
async-only — el único modo MVP1 tras §10. Implementar el worker de 5b sin
declarar la decisión sería reabrir un contrato de Fase 3 en silencio, igual que
el riesgo que DHU-025 evitó para `ZoneVehicleCount`.

### Hallazgo (citas literales)

**§4.4 Cambio 2 (Sesión 1), `tth-08-fase1-diseno.md:476-485`** — el caller persiste:
...
>
> Razón: separar cómputo de persistencia es lo que prescribe DDD ortodoxo. La
> auditoría de Fase 0 §3.2 identificó la duplicación masiva entre sync y async
> aggregator: ambos contenían cómputo idéntico + persistencia mezclada. Separar
> permite (a) una función pura de cómputo reusable entre los dos modos, (b)
> tests sin mockear el repository, (c) cumplir Single Responsibility."

**Docstring del Protocol `AsyncAggregator` (`domain/protocols.py:72-73`)** — caller
persiste, en el código:

> "The caller (typically a use case in `application/`) is responsible for
> persistence via the `TrafficRepository` Protocol."

**§11 (Sesión 3), `tth-08-fase1-diseno.md:1218-1241`** — el worker persiste:

> "11.1 Regla 1 — errores de `save`: **El worker** captura la excepción, hace
> `logger.exception` … incrementa el contador `aggregation_errors`, y continúa
> el loop. … El worker nunca mata el pipeline principal de captura/detección:
> un fallo de persistencia degrada la serie histórica, no la operación en vivo."
>
> "11.2 Regla 2 — cola llena (DROP-NEWEST): … ante `asyncio.QueueFull` se
> descarta la ventana **entrante** … El worker de persistencia no tiene
> [el fallback del broadcaster SSE] — la BD es el destino, no una fuente
> alterna. … La cola del aggregator se dimensiona más grande … default sugerido
> **~256**."

§11 modela explícitamente "**el worker**" como el que captura errores de `save`,
lo llama "**worker de persistencia**", y habla de "**la cola del aggregator**".
El lenguaje literal de §11 es **worker-persiste**, en oposición directa a §4.4
Cambio 2.

**Sesión 3 no rectificó §4.4 Cambio 2 ni el docstring del Protocol**. La única
nota de Sesión 3 sobre §4.4 declara supersede de §6.6 (eliminación de
`SyncAggregator`), pero deja el principio "caller persiste" vigente
literalmente en dos fuentes oficiales.

### Opciones evaluadas

| Opción | Descripción | Trade-off |
|---|---|---|
| **(a)** (elegida) | Worker-persiste. §11 supersede tácita explicitada vía este DHU + corrección de las dos fuentes contradictorias. El worker del `AsyncAggregator` corre la persistencia con §11.1/§11.2. | Cumple la letra de §11 directamente. Las garantías que motivaron §4.4 Cambio 2 sobreviven: cómputo separado en `_compute.py` (5a), repositorio inyectado por constructor, tests con repo fake. |
| (b) | Caller-persiste. AsyncAggregator solo computa; el use case del caller corre un thread/coroutine persister que aplica §11. | Conserva §4.4 + Protocol al pie, pero **no cambia el runtime** — el caller termina necesitando un worker persister desacoplado igual que (a), porque persistir síncrono bloquearía el pipeline. Pura indirección sin ganancia operativa, contra la letra de §11 que habla del worker. |
| (c) | `add+flush` síncronos sin worker async dentro del aggregator. | Elimina el async-real del Protocol, mata el aislamiento de I/O que §6.6 conservó a propósito. Descartada. |

### Decisión

**Se elige la opción (a).** Justificación:

1. **(a) y (b) no difieren en runtime.** Ambas necesitan un worker async que
   persiste con §11 — si el caller persistiera síncronamente bloquearía el
   pipeline (operación en vivo), que es exactamente lo que §11.1 protege contra.
   (b) solo agrega una capa organizacional para satisfacer la letra de §4.4 sin
   ganancia operativa. **Mismo comportamiento, más arquitectura.**
2. **Single Responsibility de §4.4 es más débil acá de lo que aparenta.** Aplica
   a entidades de dominio. `AsyncAggregator` no es entidad de dominio: es
   componente de aplicación con lifecycle (worker thread, `force_flush`,
   `stop`). Que un componente de pipeline async sea responsable de "agregar y
   darle durabilidad a lo agregado" es una responsabilidad cohesiva, y como
   depende del Protocol `TrafficRepository` (abstracción inyectada, no Postgres
   concreto), **Dependency Inversion queda intacto**. (a) no viola DDD; lo
   cumple por la vía correcta para esta clase de componente.
3. **Las garantías de §4.4 que importan sobreviven en (a)**:
   - Función pura de cómputo reusable: `_compute.py` ya extraída en 5a.
   - Tests sin mockear el repo real: el repo se inyecta por constructor, los
     tests inyectan un fake.
   - Separación cómputo/persistencia: la pura calcula, el worker delega el
     save a la abstracción `TrafficRepository`.
   Lo único que se relaja es "el caller orquesta el save explícitamente",
   que en async-only es difícil de combinar con un worker thread continuo.

### Refinamiento de implementación: save y output-queue independientes

El plan inicial decía "el worker persiste antes de pushear a output queue,
`flush()` retorna lo persistido". **Se refina: `flush()` retorna lo computado,
no lo persistido.** La razón viene de la propia §11.1:

> "un fallo de persistencia degrada la serie histórica, no la operación en vivo".

La output queue es la **operación en vivo** (telemetría). El `save` es la
**serie histórica**. §11.1 los declara independientes. Por lo tanto:

- `compute → save best-effort (§11.1: try/except + aggregation_errors + continue) e independientemente push a output queue best-effort (§11.2: drop-newest + data_dropped)`.
- **Un fallo de save no saca el item de la output queue** — la operación en
  vivo sobrevive, exactamente lo que §11.1 promete.
- `flush()` drena la output queue = lo computado-y-no-dropeado, alineado con
  el docstring del Protocol que dice "already computed and waiting in the
  output queue" (no dice "persisted"). Bonus: el docstring de `flush()` no
  necesita corrección — solo la cláusula "caller persiste".

### Alcance del cambio

El registro se aplica en **tres lugares** simultáneamente para que el contrato
quede coherente en registro, código y diseño — no un DHU que hay que ir a
buscar mientras dos fuentes oficiales dicen lo opuesto.

| Lugar | Cambio | Razón |
|---|---|---|
| Este DHU-026 en `DECISIONS_HU.md` | Fuente canónica de la decisión (paralelo a DHU-025). | Trazabilidad en el registro de decisiones. |
| Docstring del Protocol `AsyncAggregator` en `edge_device/src/vision/domain/protocols.py` | Reemplazar la frase literal "the caller … is responsible for persistence" por la cláusula "worker-persiste en MVP1, §11 / DHU-026" + nota sobre save y output-queue independientes. | Hoy el código contradice lo que el worker va a hacer línea por línea. Dejarlo así es deuda activa durante toda 5b. Reabre Fase 3 únicamente para esta corrección, autorizado por este DHU. |
| Nota cruzada en `tth-08-fase1-diseno.md` §4.4 Cambio 2 | Una línea apuntando a DHU-026 para que quien lea §4.4 sepa que el caller-persiste quedó superseded en modo async. | El conflicto vive en tres lugares; si no se anota en §4.4 queda mintiendo. |

### Trazabilidad

- Fuente del hallazgo: paso 0 de 5b TTH-08, sesión claude.ai 2026-05-28 con
  Cesar, después del cierre limpio de 5a (DHU-025 + ZoneVehicleCount.occupancy
  + `_compute.py`).
- Plan de Fase 5, §5b paso 0 (`/Users/rasec/.claude/plans/fase-4-de-tth-08-splendid-hoare.md`).
- Decisión cerrada en mismo turno; salida (a) elegida sobre (b) por equivalencia
  de runtime y sobre (c) por preservación del aislamiento async de §6.6.

### Documentos relacionados

- `documentation/docs/tth-08-fase1-diseno.md` §4.4 Cambio 2 (Sesión 1) + §11
  (Sesión 3) + nota cruzada que este DHU agrega al §4.4.
- `edge_device/src/vision/domain/protocols.py` (docstring del `AsyncAggregator`
  a corregir).
...
## DHU-028 — Ampliación del Product Backlog: vista panorámica de red por tramo de vía (HU-22 + TTH-12)

**Fecha:** 2026-06-02.
**Estado:** Cerrada.
**Amplía:** el componente funcional del Product Backlog declarado completo en DHU-017 (HU-01 a HU-21 + TTH-01 a TTH-11, cerrado 2026-05-16). Tras esta decisión: HU-01 a HU-22 + TTH-01 a TTH-12.

### Contexto

Durante el trabajo de implementación posterior al cierre del backlog (2026-06-02) se identificó una necesidad de producto no cubierta por ninguna HU existente: una **vista panorámica de la congestión de toda la red vial sobre un mapa geográfico**, donde el Operador pueda leer de un vistazo dónde se está formando congestión sin revisar intersección por intersección.

El catálogo cerrado cubría el monitoreo del Operador a nivel de **intersección individual**:

- HU-02 — estado actual de una intersección.
- HU-03 — predicción de congestión por **acceso** de la intersección.
- HU-04 — vista combinada (estado + predicción) por **acceso** de la intersección.

Ninguna de estas entrega una vista de red completa. HU-03 y HU-04, además, están redactadas con la unidad de análisis **"acceso de la intersección"** (las direcciones de aproximación a una intersección concreta). Esa unidad es correcta para una vista de detalle de intersección, pero no permite representar la congestión de manera continua sobre la geografía de la red, que es lo que un mapa de calor requiere.

Adicionalmente, durante la evolución técnica del proyecto la unidad natural de análisis del estado del tráfico se consolidó como el **tramo de vía (arista del grafo vial)**: el modelo de datos (`graph_edges`, `waze_jams` con `edge_id`), el dataset de simulación y los experimentos de predicción operan por arista. Existe, por tanto, una distancia entre la unidad de las HU de detalle redactadas tempranamente ("acceso") y la unidad de datos efectiva del sistema ("arista"). Esta distancia no invalida HU-03/HU-04 —siguen siendo válidas como vista de detalle de intersección— pero sí significa que una vista de red debe redactarse explícitamente por arista, no derivarse de las HU por acceso.

### Decisión

1. **Se amplía el Product Backlog con HU-22** ("Como Operador, quiero ver sobre un mapa de la red un indicador de calor del nivel de congestión actual de cada tramo de vía, actualizándose en vivo, para identificar de un vistazo dónde se forma congestión y priorizar la intervención"). HU-22 se ubica en el Bloque B (monitoreo del Operador) como ampliación, y se redacta conforme a las reglas metodológicas vigentes (DHU-003, DHU-005 Caso A, DHU-006, DHU-007, DHU-018).

2. **Se amplía el catálogo de TTH con TTH-12** (infraestructura de datos de congestión por arista: geometría de la red poblada + feed de estado por arista persistido en `waze_jams` + endpoints), que habilita a HU-22. TTH-12 se implementa antes que HU-22 (sin datos no hay vista).

3. **Unidad de análisis de la vista de red:** HU-22 usa el **tramo de vía (arista)** como unidad, no el acceso de intersección. Esta es la unidad que permite la representación geográfica continua del mapa de calor y es coherente con la unidad de datos efectiva del sistema.

4. **HU-03 y HU-04 NO se reabren ni se reescriben.** Se reinterpretan como **vista de detalle de intersección** (el nivel de zoom donde el Operador examina una intersección concreta y sus accesos), complementaria a la **vista panorámica de red** de HU-22 (el nivel de zoom de toda la red). Son dos niveles de zoom del mismo sistema de monitoreo, no sustitutos. La eventual transición por zoom entre ambas vistas se redactará como ampliación futura cuando ambas estén implementadas.

5. **Alcance de la V1 de HU-22:** estado de congestión **actual** por arista. La proyección **predicha** sobre el mismo mapa queda como extensión futura, con su propia TTH de predicción por arista y su registro de decisión cuando se aborde.

6. **Fuente de datos de la V1 (registrada en TTH-12, no en la HU):** la HU es agnóstica a la fuente (DHU-006). La V1 usa SUMO como stand-in del feed de Waze, por replay de un dataset pre-generado, detrás de una interfaz de feed que en producción llenaría un ingestor de Waze real. La tubería persiste en `waze_jams`, de modo que la sustitución de la fuente no altera los consumidores.

### Justificación

- **Necesidad de producto genuina y no cubierta:** la vista panorámica de red es el caso de uso de "dónde intervengo primero" del Operador, distinto del "qué pasa en esta intersección" que cubren HU-02/03/04. No existe HU que lo cubra; corresponde redactarla, no forzar las existentes.

- **Patrón de ampliación ya establecido:** el proyecto ya amplió catálogos cerrados cuando apareció una necesidad legítima — DHU-015 amplió el Bloque E de 4 a 5 TTH al identificar TTH-11. Ampliar de 21 a 22 HU y de 11 a 12 TTH sigue ese mismo patrón, con su decisión registrada para preservar trazabilidad.

- **Honestidad de la unidad de análisis:** registrar explícitamente que la vista de red es por arista (y por qué) evita la confusión de que HU-22 "contradiga" a HU-03/HU-04. La distinción acceso (detalle) vs arista (red) queda documentada, no implícita.

- **Separación HU/TTH respetada (DHU-004):** el valor para el Operador (el mapa que ve) es HU; la infraestructura de datos que lo alimenta (geometría, feed, persistencia) es TTH. No se mezcla trabajo técnico en la voz del usuario.

### Consecuencias

- Se crea `HU-22_mapa_congestion_red.md` (o se integra HU-22 al documento de Bloque B según la convención de ubicación vigente).
- Se agrega TTH-12 a `TAREAS_TECNICAS_HABILITADORAS.md` y al índice de TTH.
- El conteo del backlog pasa de "21 HU + 11 TTH" a "22 HU + 12 TTH". Los documentos que citan el conteo cerrado (encabezados de `HU_BLOQUE_B.md`, `spec.md`, `constitution.md` si aplica) se actualizan para reflejar la ampliación, citando esta DHU como fuente.
- HU-03 y HU-04 reciben (de forma aditiva, sin reescribir sus CAs) una nota que las identifica como vista de detalle de intersección, complementaria a HU-22.
- Se difieren explícitamente como extensiones futuras: la capa de predicción por arista sobre el mapa, el upgrade a SUMO en vivo, el ingestor de Waze real, y la transición por zoom entre vista de red y vista de detalle.

### Lo que NO cambia con DHU-028

- Las decisiones DHU-001 a DHU-027 mantienen su contenido sustantivo; DHU-028 las usa como base sin reabrir ninguna.
- Las 21 HU previas (HU-01 a HU-21) no se reescriben. HU-03/HU-04 reciben solo una nota aditiva de complementariedad.
- Las 11 TTH previas (TTH-01 a TTH-11) no se modifican.
- Las decisiones técnicas D-001 a D-013 se mantienen. En particular, la decisión de no adoptar el STGNN (D-011) se mantiene: la capa de predicción por arista, cuando se aborde, decidirá su predictor en su propio momento y no es parte de esta ampliación.
- El alcance del producto (Personas, Objetivos, Journeys, Visión) se mantiene; HU-22 sirve a la Persona Operador ya existente y al Objetivo de monitoreo operativo ya declarado.

---

## DHU-029 — Ampliación del Product Backlog: barrido temporal de la congestión de la red (HU-23 + TTH-13)

**Fecha:** 2026-06-02.
**Estado:** Cerrada.
**Amplía:** la ampliación cerrada en DHU-028 (HU-22 + TTH-12, vista panorámica de congestión de la red). Tras esta decisión: HU-01 a HU-23 + TTH-01 a TTH-13.

### Contexto

Durante el análisis de evolución de la vista panorámica de red (HU-22, mergeada) se identificó una necesidad de producto del Operador no cubierta por ninguna HU existente: poder **recorrer en el tiempo** la congestión de la red sobre el mismo mapa geográfico, para entender cómo se formó la situación que está observando en vivo antes de decidir una intervención.

HU-22 entrega la congestión **actual** de la red, actualizándose en vivo. Su valor es "dónde intervengo ahora". Lo que no cubre es la pregunta inmediatamente anterior a la intervención: *"¿cómo evolucionó esta congestión en las horas previas — viene creciendo o se está disipando?"*. Esa lectura del recorrido reciente es contexto operativo del Operador, no análisis de planificación del Gerente.

La distinción con las HU del Gerente es deliberada y se registra aquí para evitar confusión:

- **HU-16/HU-17/HU-18** (Gerente) cubren análisis de periodos prolongados (semana/mes), comparativas entre periodos y drill-down de KPIs agregados, sobre el histórico observado real del sistema. Su unidad es el indicador de desempeño (tiempo de espera, cola, throughput, demora) y su medio es el dashboard, no el mapa geográfico por arista.
- **HU-23** (esta ampliación, Operador) cubre el recorrido temporal de la **congestión por tramo de vía sobre el mapa**, en el horizonte de un día, como contexto inmediato a la operación en vivo. Su unidad es la arista y su medio es el mismo mapa de HU-22.

No hay solape: ninguna HU del Gerente representa la congestión geográfica por arista recorrible en el tiempo. La unidad (arista) y el medio (mapa de calor) son los de HU-22, extendidos con un eje temporal.

### Decisión

1. **Se amplía el Product Backlog con HU-23** ("Como Operador, quiero recorrer sobre el mapa de la red la evolución de la congestión de cada tramo de vía a lo largo del día mediante un control temporal, para entender cómo se formó la situación actual antes de decidir dónde intervenir"). HU-23 se integra al documento del Bloque B (monitoreo del Operador), inmediatamente después de HU-22, conforme a la convención de ubicación aplicada en DHU-028 (HU-22 se integró a `HU_BLOQUE_B.md`, no a archivo suelto).

2. **Se amplía el catálogo de TTH con TTH-13** (infraestructura de consulta histórica de congestión por arista: lectura de estado por arista en un instante arbitrario + reporte de rango temporal disponible), que habilita a HU-23. TTH-13 se implementa antes que HU-23 (sin la consulta por instante no hay barrido que pintar).

3. **Unidad y medio de HU-23:** el **tramo de vía (arista)** sobre el **mapa geográfico**, idénticos a HU-22. HU-23 reutiliza la geometría (CT-12.1/12.2) y la persistencia en `waze_jams` (CT-12.5) de TTH-12; agrega únicamente la dimensión temporal de lectura.

4. **Alcance temporal de HU-23 (V1):** recorrido del **estado de congestión histórico y actual** del día, en el rango de 0 a 24 horas. La proyección **predicha** a futuro sobre el mismo eje temporal (p. ej. los próximos 30 minutos) queda **fuera de alcance** de HU-23 y se aborda como extensión predictiva, con su propia HU y su propia TTH de predicción por arista, cuando se decida y sirva el predictor correspondiente.

5. **Cobertura parcial del día en curso:** cuando el día seleccionado es el día actual y los datos solo cubren hasta cierta hora, el control temporal se **bloquea** más allá de la última hora con datos disponibles, en lugar de permitir el recorrido sobre un rango vacío. El rango disponible lo reporta el backend (TTH-13); el frontend no asume cobertura.

6. **Honestidad de la fuente (V1):** HU-23 es agnóstica a la fuente (DHU-006), igual que HU-22. En la implementación de V1, los datos recorridos provienen del replay de un día simulado (SUMO, stand-in del feed de Waze, conforme a TTH-12). La vista en modo de recorrido temporal **rotula explícitamente** que se trata de un día de muestra/replay, de modo que el recorrido histórico no se presente como histórico operativo real. Esto es coherente con el principio de no presentar datos de simulación como operativos.

### Justificación

- **Necesidad de producto genuina y no cubierta:** el contexto temporal inmediato ("cómo llegó la red a este estado") es parte del juicio del Operador antes de intervenir, distinto del análisis de periodos del Gerente. No existe HU que lo cubra a nivel de mapa geográfico por arista; corresponde redactarla, no forzar HU-22 (que se define como "actual/en vivo") ni las HU del Gerente (que son dashboard de KPIs).

- **Coherencia de la voz de HU-22:** colgar un control de recorrido temporal histórico bajo HU-22 —cuya voz entera es "estado actual, en vivo, para intervenir ahora"— habría contradicho su propio "para". Redactar HU-23 como HU separada mantiene la honestidad de ambas voces: HU-22 es el ahora, HU-23 es el recorrido del día.

- **Patrón de ampliación ya establecido:** DHU-028 amplió un backlog declarado completo cuando apareció una necesidad legítima (HU-22 + TTH-12). Ampliar de 22 a 23 HU y de 12 a 13 TTH sigue ese patrón, con su decisión registrada para preservar trazabilidad.

- **Reutilización de la infraestructura de TTH-12:** HU-23 no requiere infraestructura nueva de geometría ni de persistencia; reutiliza la de TTH-12. TTH-13 agrega solo la capacidad de lectura por instante arbitrario y el reporte de rango, que TTH-12 no proveía (su endpoint de estado está cableado al snapshot más reciente).

- **Separación HU/TTH respetada (DHU-004):** el valor para el Operador (el recorrido que ve) es HU; la capacidad técnica de consultar el estado por instante y reportar el rango disponible es TTH. No se mezcla trabajo técnico en la voz del usuario.

### Consecuencias

- Se agrega la ficha de HU-23 a `HU_BLOQUE_B.md`, inmediatamente después de HU-22, y se actualiza el conteo en el encabezado del bloque.
- Se agrega TTH-13 a `TAREAS_TECNICAS_HABILITADORAS.md` y a su índice de TTH.
- El conteo del backlog pasa de "22 HU + 12 TTH" a "23 HU + 13 TTH". Los documentos de backlog que citan el conteo se actualizan citando esta DHU como fuente.
- **Propagación a artefactos Spec Kit diferida (coherente con DHU-028):** `spec.md` y `tasks.md` mantienen sus encabezados canónicos en el conteo del que se derivaron ("21 HU + 11 TTH"); la nota de diferimiento de `spec.md` (que ya reconoce la ampliación HU-22/TTH-12 de DHU-028) se **amplía** para reconocer también HU-23/TTH-13, manteniendo la re-derivación Spec Kit como deuda explícita y consciente, no como reescritura del conteo. No se reabre la re-derivación en esta decisión.
- `constitution.md` no declara conteo de backlog (verificado: solo referencia rangos de decisiones metodológicas), por lo que no recibe actualización de conteo por esta DHU.
- Se difiere explícitamente como extensión futura: la **capa de predicción por arista sobre el eje temporal del mapa** (proyección a +30 min y horizontes futuros). Esta extensión se registra dimensionada con el hallazgo de la auditoría 2026-06-02 (ver "Lo que NO cambia con DHU-029").

### Lo que NO cambia con DHU-029

- Las decisiones DHU-001 a DHU-028 mantienen su contenido sustantivo; DHU-029 las usa como base sin reabrir ninguna.
- Las 22 HU previas (HU-01 a HU-22) no se reescriben. En particular, HU-22 conserva intacta su voz "estado actual, en vivo"; HU-23 es complementaria, no la modifica.
- Las 12 TTH previas (TTH-01 a TTH-12) no se modifican. TTH-13 reutiliza la infraestructura de TTH-12 sin alterar sus contratos (CT-12.x).
- Las decisiones técnicas D-001 a D-013 se mantienen. En particular, **la decisión de no adoptar el STGNN (D-011) se mantiene**: la capa de predicción por arista, cuando se aborde, decidirá su predictor en su propio momento y no es parte de esta ampliación.
- **Dimensionamiento de la extensión predictiva futura (auditoría 2026-06-02, para la TTH predictiva cuando se redacte):** la infraestructura de grafo por arista ya existe — el grafo del predictor es *edge-as-node* (375 nodos = 375 aristas SUMO, alineadas con `graph_edges`), confirmado en `miraflores_graph_lcc_mapping.json`. Existen dos modelos por arista entrenados offline (`miraflores_stgnn.pt` y `miraflores_gru_baseline.pt`), pero **ambos predicen `timeLoss` en segundos (regresión)**, no el nivel de congestión 0-5 que el mapa pinta. El único modelo que emite 0-5 (`gru_{N,S,E,W}.pt`, servido en `/predictions/predict`) es por dirección N/S/E/W de intersección, sin relación con el grafo. Por tanto, la extensión predictiva no es "servir un modelo existente": requiere resolver el puente entre la variable predicha (timeLoss) y la escala del mapa (0-5 por velocidad, D-009) — o entrenar un predictor por arista que emita 0-5 directamente — y la decisión sobre el predictor (incluido si reabrir D-011) es propia de esa fase. Este hallazgo se registra aquí para dimensionar la fase, no para decidirla.
- El alcance del producto (Personas, Objetivos, Journeys, Visión) se mantiene; HU-23 sirve a la Persona Operador ya existente y al Objetivo de monitoreo operativo ya declarado.
...
| Bloque B | HU-02 a HU-09, HU-22, HU-23 | TTH-12, TTH-13 (ampliaciones, DHU-028/DHU-029) | DHU-003, DHU-005 (refinada con A y B), DHU-006, DHU-007, DHU-028, DHU-029 |
...
| Bloque E | (ninguna HU operativa) | TTH-07, TTH-08, TTH-09, TTH-10, TTH-11 | DHU-015 (clasificación HU/TTH del Bloque E con ampliación 4 → 5 TTH durante la redacción); DHU-024 (encuadre cerrado del refactor de TTH-08: alcance 11 CTs operativos, arquitectura DDD completa, borrado Alembic de vision_tracks/vision_flows, C7.6 resuelta dentro del refactor, F41 fuera de scope, levantamiento de la regla CLAUDE.md planificado al primer commit del sprint); DHU-025 (extensión de `ZoneVehicleCount` con `occupancy: float` + cómputo geométrico de `mean_occupancy` por unión, reapertura formal de Fase 3/4a en Fase 5 de TTH-08); DHU-026 (persistencia en el worker del `AsyncAggregator` para Fase 5 de TTH-08: §11 supersede a §4.4 Cambio 2 en async-only MVP1, save y output-queue como paths independientes, reapertura mínima de Fase 3 para corregir docstring del Protocol + nota cruzada en §4.4) |
...
- `HU_BLOQUE_B.md` — Bloque B del Product Backlog (10 HUs, 7 MVP1 + 1 MVP2 + 2 ampliaciones: HU-22, HU-23).
...
- `TAREAS_TECNICAS_HABILITADORAS.md` — TTH-01 a TTH-13.
