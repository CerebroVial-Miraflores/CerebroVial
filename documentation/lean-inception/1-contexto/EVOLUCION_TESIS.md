...
> **F41 reafirmada en TTH-08 F9 (2026-05-29).** El cierre del sprint TTH-08 confirma F41 como integración futura sin cambios estructurales: ámbito reservado = cálculo de `queue` (columna `vision_aggregates.queue` nullable en MVP1), calibración direccional por zona, tipos adicionales en `vehicles_by_type` y enriquecimiento del SSE con `camera.street_monitored` si aparece consumidor. Ver `documentation/contracts/vision_contract.md` §6 (integración futura con módulo predictivo) y `DECISIONS_HU.md` DHU-024 §5 + addendum F9.
