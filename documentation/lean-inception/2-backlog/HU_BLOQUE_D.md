...
- **CA-14.1:** Dado que el modelo predictivo del sistema genera una predicción para un horizonte futuro, cuando esa predicción se entrega a sus consumidores (Operador, motor adaptativo, otros), entonces el sistema persiste la predicción de forma durable con al menos: marca de tiempo de generación, marca de tiempo del horizonte al que la predicción aplica, valor predicho del ratio continuo, nivel discreto 0-5 correspondiente.
  ...
  > **Nota de reconciliación (D-005, TTH-09 2026-05-31).** "valor predicho del ratio continuo" en CA-14.1 (y, por extensión, CA-14.2/CA-14.3) asume un regresor de ratio; la producción de TTH-09 es un **clasificador GRU multi-output** sin salida de ratio (ver `documentation/contracts/prediction_contract.md` §4/§6 y CT-09.5/CT-09.6 reconciliadas). El registro persiste el **`level` discreto (`argmax`) + `probs`** (tabla `predictions`); HU-14 compara el `level` predicho contra el `jam_level` observado. MAE/RMSE-sobre-ratio se reemplaza por métricas de clasificación (accuracy, F1-macro, matriz 6×6, MAE ordinal). **Scope:** la observación real (CA-14.2), el join predicho-vs-real y la ventana de 24h son HU-14, fuera de TTH-09 Fase 5 (que solo produce la fuente de datos vía CT-09.5). Texto original conservado como rastro.
...
Esta sesión cerró el Bloque D. A la fecha actual, los Bloques E y F del MVP1 están cerrados y el MVP2 también está cerrado (DHU-017, 2026-05-16); **con el cierre del MVP2, la redacción del Product Backlog del proyecto queda completa en su componente funcional: 23 HU operativas (HU-01 a HU-23) y 13 TTH (TTH-01 a TTH-13), tras las ampliaciones DHU-028 (HU-22 + TTH-12) y DHU-029 (HU-23 + TTH-13).** HU-20 (comparativa vs modelo de respaldo) extiende HU-14 mediante persistencia paralela inglobada. Los siguientes pasos del proyecto, en sesiones futuras:
