...
[ ] **DEUDA-ZONAS-ONDEMAND** — El alta on-demand de cámaras (`_build_camera_config`, `edge_device/src/vision/presentation/api/routes/cameras.py:51`) arma el cfg desde cero y toma `zones` del body. Tanto el frontend (`CameraDetailView.tsx:247`) como cualquier POST mandan `zones: {}`, y `_build_camera_config` NO mergea `conf/vision/default.yaml`, así que las `zone1`–`zone4` genéricas tampoco entran. Sin zonas el `zone_counter` no emite `TrafficData` y `vision_aggregates` nunca recibe filas en el flujo on-demand — aunque la persistencia esté "wired". **Bloquea E21**: correr una cámara no basta. Salidas (dev/calibración): (a) pasar zonas reales en el body; (b) inyectar zonas en el path on-demand; (c) fallback a `default.yaml` cuando el body manda `{}`. Definir zonas reales de una cámara usa el script de calibración interactivo (`interaction.py`, ver C7.7). **Cross-ref:** E21; C7.7. **Actualización (Fase 3, 2026-06-12):** el fallback zone-less (`_compute._compute_zoneless`) emite un `TrafficData` (zone_id `all`) con `zones: {}` para alimentar el panel en vivo (conteo/flujo/velocidad), PERO ese sentinel se **EXCLUYE de la persistencia** (filtro `td.zone_id.value != ZONELESS_ZONE_ID` en `async_aggregator._process_window`, fix Fase 3): presencia extrapolada no calibrada NO se guarda como agregado medido. Por eso **E21 NO se ablanda**: el flujo on-demand sigue SIN generar filas en `vision_aggregates` hasta que haya zonas calibradas reales — que es la intención original de esta deuda. **Cross-ref:** DEUDA-OCUPACION-SIN-ZONA; DEUDA-MULTIZONA-AGREGADO.

 [ ] **DEUDA-OCUPACION-SIN-ZONA** — Con el fallback zone-less (Fase 3, `edge_device/src/vision/application/aggregators/_compute.py:_compute_zoneless`), el panel del operador muestra **conteo, flujo y velocidad reales** sobre toda la imagen (misma fuente que el overlay → coherente, mata el "0 con boxes"), PERO **congestión y densidad quedan en 0**: la ocupación es relativa a un polígono y sin zonas calibradas no se computa (`mean_occupancy=0.0`, `density_vehicles_per_km=None`). Es **decisión consciente** del plan de Fase 3 (no bug): sin pseudo-zona full-frame, sin cv2 en la función pura. Límite honesto, cubierto por el caveat textual del panel ("presencia extrapolada, no aforo calibrado") y la política real-con-caveat. **Trigger de cierre:** calibrar zonas por cámara/tramo (workstream de calibración, `interaction.py`); al haber zonas, el camino por-zona computa occupancy real y las barras de congestión/densidad cobran vida. **Cross-ref:** DEUDA-ZONAS-ONDEMAND; C7.7; D-018 (revisión Fase 2); Fase 3.

 [ ] **DEUDA-MULTIZONA-AGREGADO** — Semántica **FIJADA** (plan Fase 3): cuando una cámara tenga múltiples zonas calibradas (asociadas a tramos/corredores), el número único del panel ("Vehículos detectados") debe mostrar el **TOTAL AGREGADO** entre zonas, no la última. Hoy `compute_traffic_data` emite un `TrafficData` por zona y el broadcaster publica uno por uno; `useVisionStream` guarda el ÚLTIMO → con multi-zona el panel mostraría solo la última zona. **No se ejercita hoy**: ninguna cámara tiene zonas calibradas y el fallback zone-less emite un único `TrafficData` (zone `all`). **Implementación del agregado pendiente (fuera de scope Fase 3):** sumar las N zonas, en el edge (emitir un payload agregado) o en el front (acumular los per-zona en `useVisionStream`). **Trigger:** cuando se calibren zonas múltiples en una cámara. **Cross-ref:** DEUDA-OCUPACION-SIN-ZONA; DEUDA-ZONAS-ONDEMAND; Fase 3.
...
DEUDAS PACING/ESCALADO VISIÓN — fluidez del feed (2026-06-14)
Detalle y evidencia: `documentation/docs/20260614_vision_decode_pacing.md`.

 [ ] **DEUDA-VISION-ESCALADO-NCAMARAS** — La ventana de gather del `BatchInferenceWorker` quedó **fija en 5ms** (óptima para 1-2 cámaras) y el worker es **serial** (~29 inferencias/s totales en MPS): no escala a N cámaras a 25fps (las 11 del corredor lo saturan). El `analyze_fps=25` por default aplica también a Docker-CPU, donde el margen es menor (caveat nativo-CPU vs Docker-CPU); override por env `VISION_ANALYZE_FPS`. **Trigger:** >~2-3 cámaras activas a la vez. **Qué falta:** batching adaptativo (cortar el gather al tener 1 frame por cámara activa, en vez del cap fijo). **Cross-ref:** `20260614_vision_decode_pacing.md` § escalado N-cámaras.
 [ ] **TRABAJO-FUTURO-MULTIPROC-VISION** — Separar decode (N procesos CPU) de inferencia (1 proceso GPU) por memoria compartida, para el escalado N-cámaras. **NO es para el cuello de la entrega bursty** (ya resuelto con `-re`): la hipótesis de contención de GIL fue medida y **refutada**. Es para cuando el cuello sea realmente el cómputo con muchas cámaras. Material GIL/multiproceso ya recopilado. **Cross-ref:** `20260614_vision_decode_pacing.md` § trabajo futuro multiprocessing.